...
def save_images(batch_image, batch_joints, batch_joints_vis, output_dir, meta, iteration):
    ...
    gt_file = '/model/omnipose/data/pictogram/annot/valid.json'
    ...
    for i in range(batch_joints.shape[0]):
        ...
        if Counter(images_done)[file_name[i]] > 0:
            filename = 'output/mpii/omnipose/omnipose_w48_256x256yaml/val_images/'+file_name[i][-13:]
            print(filename)
        else:
        ...
        for k, link_pair in enumerate(link_pairs):
            if gt[i_gt]['joint_vis'][link_pair[0]] == 1 and gt[i_gt]['joint_vis'][link_pair[1]] == 1:
                line = mlines.Line2D(
